...

{
    ...
    ros::init(argc, argv, "publisher_speed_control");
    ...
    float current_limit = 0.20;

    float MAX_LINEAR = 0.9;

    float MAX_ANGULAR = 0.1;
...

    {
        ...
        bool r2_now = (map.r2 < -10000);

        bool l2_now = (map.l2 < -10000);
        ...
        if (r2_now && !prev_r2)
        {
            ...
            if (current_limit > MAX_LINEAR)
                current_limit = MAX_LINEAR;

            ROS_INFO("Speed Up: %.2f", current_limit);
        }
        ...
        if (l2_now && !prev_l2)
        {
            ...
            ROS_INFO("Speed Down: %.2f", current_limit);
        }
